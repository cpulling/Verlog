#!/bin/bash
# TOOL-CALL smoke variant of launch_qwen3_slim.sh — Qwen3-0.6B on ARC with
# Hermes tool-calling instead of the XML-tag grammar. Otherwise identical to
# launch_qwen3_slim.sh so any behavior/* delta between the two arms isolates
# format effects, not hyperparameter drift.
#
...
export ARC_ACTION_MODE=tool_calls
export GUIDED_REGEX=""
TOOL_CONFIG_PATH=verl/envs/environments/arc_game/tool_config/arc_tools.yaml
...
export ARC_TRAJECTORY_LOG="$LOG_DIR/qwen3_0p6b_tools_${TAG}.trajectory.jsonl"
: > "$ARC_TRAJECTORY_LOG"
...
LOGF="$LOG_DIR/qwen3_0p6b_tools_${TAG}.log"
...
DEBUG_FILE="$LOG_DIR/qwen3_0p6b_tools_${TAG}.debug.txt"
{
